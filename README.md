...
Assumptions
All SQL queries target year 2021 for Hotel and 2023 for Clinic (parameterised via SET @target_year).
item_quantity can be fractional (DECIMAL) as shown in sample data (0.5).
Profit = Revenue − Expenses; negative profit = Not-Profitable.
For Q4/Q5 in Hotel: ties are handled by RANK (all tied items get the same rank).
Spreadsheet formulas use INDEX-MATCH instead of VLOOKUP for robustness across column insertions
